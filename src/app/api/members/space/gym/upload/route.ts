...
export async function POST(req: Request) {
  const limited = rateLimitResponse(req, "gym-upload", 24, 15 * 60 * 1000);
  ...
  if (!memberCanAccess(space, "gymLog")) {
    return NextResponse.json(
      { error: "Gym is locked on your plan" },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    if (!isVideo && !isImage) {
      return NextResponse.json(
        { error: "Only photos or one short video are allowed" },
        { status: 400 }
      );
    }
    ...
    if (isVideo && file.size > MAX_VIDEO_BYTES) {
      return NextResponse.json(
        { error: "Video must be under 25 MB — keep it short" },
        { status: 400 }
      );
    }
    ...
    const saved = await saveUploadFile(
      buffer,
      file.name || (isVideo ? "workout.mp4" : "workout.jpg"),
      type || undefined
    );
    return NextResponse.json({
      url: saved.url,
