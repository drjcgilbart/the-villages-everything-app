...
const FIT_CLUBS = [
  { name: "Colony Cottage Fit Club", rec: "Colony Cottage", phone: "352-750-5282" },
  { name: "Ezell Fit Club", rec: "Ezell", phone: "352-674-1859" },
  { name: "Fenney Fit Club", rec: "Fenney", phone: "352-674-8464" },
  { name: "Laurel Manor Fit Club", rec: "Laurel Manor", phone: "352-751-7101" },
  { name: "Mulberry Grove Fit Club", rec: "Mulberry Grove", phone: "352-674-1829" },
  { name: "Olympia Fit Club", rec: "Olympia / Eastport", phone: "352-674-1944" },
  { name: "Rohan Fit Club", rec: "Rohan", phone: "352-674-8404" },
  { name: "SeaBreeze Fit Club", rec: "SeaBreeze", phone: "352-750-0237" },
];
...
export function MySpaceGymBoard() {
  ...
  const [liftName, setLiftName] = useState("Chest press");
  ...
  const [supWhen, setSupWhen] = useState("morning");
  const [placeName, setPlaceName] = useState("");
  const [placeKind, setPlaceKind] = useState("fitclub");
  if (!ready) return <p className="panel-hint">Loading gym…</p>;

  const todayWos = value.workouts.filter((w) => w.date === today());

  function logWorkout() {
    const nSets = Math.min(10, Math.max(1, Number(sets) || 1));
    const lift: GymLift = {
      name: liftName,
      kind: "machine",
    ...
    const wo = {
      id: uid("wo"),
      date: today(),
      gymName: gymName || value.gyms.find((g) => g.id === value.homeGymId)?.name || "",
      durationMin: (Number(duration) || "") as number | "",
      felt: "",
      notes,
      exercises: [lift],
    };
    void save({ ...value, workouts: [wo, ...value.workouts].slice(0, 60) });
    setNotes("");
  }
  ...
  return (
    <div className="about-panel ms-module">
      ...
      <SubNav
        tabs={[
          { id: "today", label: "Log workout" },
          { id: "gyms", label: "Fit Clubs & gyms" },
      ...
      {tab === "today" ? (
        <>
          ...
          <h4>Today</h4>
          {todayWos.length === 0 ? (
            <p className="panel-hint">No workout logged today.</p>
          ) : (
            <ul className="ms-cal-list">
              {todayWos.map((w) => (
                <li key={w.id}>
                  <div>
                    ...
                    <span>
                      {w.gymName || "Gym"} · {w.durationMin ? `${w.durationMin} min` : ""}{" "}
                      {w.notes}
                    </span>
                  </div>
                </li>
      ...
      {tab === "gyms" ? (
        <>
          <p className="panel-hint">
            District Fit Clubs (hours often Mon–Fri 6:30am–8:00pm). Extra Fit Club fee — confirm on
            districtgov.org.
          </p>
          <ul className="ms-cal-list">
            {FIT_CLUBS.map((c) => (
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    void save({
                      ...value,
                      gyms: [
                        { id, name: c.name, kind: "fitclub", address: c.rec, notes: c.phone },
                        ...value.gyms.filter((g) => g.name !== c.name),
                      ].slice(0, 40),
                      homeGymId: value.homeGymId || id,
                    });
                  }}
                >
                  Save as mine
          ...
          >
            ...
            <div className="field">
              ...
              <select value={placeKind} onChange={(e) => setPlaceKind(e.target.value)}>
                <option value="fitclub">Fit Club</option>
                <option value="chain">Chain (PF, LA, Anytime…)</option>
            ...
            {value.gyms.map((g) => (
              <li key={g.id}>
                ...
                <button
                  type="button"
                  className="btn btn-ghost btn-sm"
                  onClick={() => void save({ ...value, homeGymId: g.id })}
                >
                  Home
                </button>
              </li>
      ...
      {tab === "history" ? (
        <ul className="ms-cal-list">
          {value.workouts.length === 0 ? (
            <p className="panel-hint">No history yet.</p>
          ) : (
            value.workouts.map((w) => (
              <li key={w.id}>
                <div>
                  <strong>
                    {w.date} · {w.exercises.map((e) => e.name).join(", ") || "Session"}
                  </strong>
                  <span>
                    {w.gymName} {w.durationMin ? `· ${w.durationMin} min` : ""} {w.notes}
                  </span>
                </div>
                <button
                  type="button"
                  ...
                  onClick={() =>
                    void save({
                      ...value,
                      workouts: value.workouts.filter((x) => x.id !== w.id),
                    })
                  }
                >
                  Delete
          ...
          >
            <div className="field">
              ...
              <input
                ...
                required
              />
              <datalist id="ms-gym-sups">
                {["Creatine", "Protein powder", "Electrolytes", "Vitamin D", "Fish oil", "Magnesium"].map(
                  (n) => (
                    <option key={n} value={n} />
                  )
                )}
            ...
            <div className="field">
              <label>Dose</label>
              <input value={supDose} onChange={(e) => setSupDose(e.target.value)} placeholder="5g" />
            </div>
            <div className="field">
              <label>When</label>
              <input value={supWhen} onChange={(e) => setSupWhen(e.target.value)} />
            </div>
            <button type="submit" className="btn btn-primary btn-sm">
              Add
            </button>
