...
import {
  GOLF_PRACTICE_TOOLS,
  GOLF_PRACTICE_TOPICS,
  GOLF_PRACTICE_VIDEOS,
  type GolfPracticeTopic,
} from "@/lib/golfPractice";
import { youtubeWatchUrl } from "@/lib/youtubeLinks";

export function MySpaceGolfPractice() {
  const [topic, setTopic] = useState<GolfPracticeTopic | "all">("all");
  const videos = useMemo(
    () =>
      topic === "all"
        ? GOLF_PRACTICE_VIDEOS
        : GOLF_PRACTICE_VIDEOS.filter((v) => v.topic === topic),
    [topic]
  );
  ...
  return (
    <>
      <p className="panel-hint">
        ...
        (leaderboard, foursomes, aces) stays free.
      </p>

      <h3>Tips &amp; tricks</h3>
      <div className="ms-photo-chips" style={{ margin: "0.5rem 0 0.85rem" }}>
        {GOLF_PRACTICE_TOPICS.map((t) => (
          <button
            key={t.id}
            type="button"
